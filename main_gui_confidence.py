...
RETRIEVAL_THRESHOLD_DEFAULT = 0.4  # cosine similarity threshold (0..1)
ASSETS_DIR = "assets"
...
class RAGIndex:
    ...
    def retrieve(self, query: str, top_k: int, max_context_chars: int) -> Tuple[str, List[Tuple[int, str]], float | None]:
        ...
        max_sim = float(np.max(sims)) if sims.size else None
        ...
        return "\n\n".join(context_parts), citations, max_sim
...
def dialog_retrieval(top_k: int, max_chars: int, threshold: float) -> Tuple[int, int, float] | None:
    layout = [
        ...
        [sg.Text("Min retrieval score threshold (0–1):"), sg.Input(str(threshold), key="-THR-", size=(6, 1))],
        [sg.Button("Save"), sg.Button("Cancel")]
    ]
...
def main():
    ...
    while True:
        ...
        # --- Demo Mode events ---
            ...
            try:
                rag_context, cites, max_sim = state.rag.retrieve(user_text, state.top_k, state.max_context_chars)
            except Exception:
                rag_context, cites, max_sim = ("", [], None)

            # Hallucination guard based on retrieval confidence
            if max_sim is not None and max_sim < state.retrieval_threshold:
                abstain_msg = "I do not have information for this enquiry."
                append_chat("🤖 A", abstain_msg + f" (confidence={max_sim:.2f} < threshold={state.retrieval_threshold:.2f})\n")
                if state.show_sources:
                    sources.update(f"Confidence: {max_sim:.2f}  |  Threshold: {state.retrieval_threshold:.2f}\n(No sufficiently relevant sources.)")
                else:
                    sources.update("")
            ...
            if (cites or max_sim is not None) and state.show_sources:
                lines = []
                if max_sim is not None:
                    lines.append(f"Confidence: {max_sim:.2f}  |  Threshold: {state.retrieval_threshold:.2f}")
                if cites:
                    lines.append("Sources:")
                    lines += [f"  [{i}] {s}" for i, s in cites]
                sources.update("\n".join(lines))
            else:
                sources.update("")
